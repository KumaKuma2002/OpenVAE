...
def plot_correlation(
    ...
    axis_scale="mm$^3$",
    save_dir="./temp_test.png",
    fixed_range=None,    # <---- 新增参数: 传入全局坐标范围
    fig_size=(6, 6),     # <---- 固定尺寸
    dpi=300,
):  
    ...
    """
    size_pred, size_gt = zip(*input_list)
    size_gt, size_pred = np.array(size_gt), np.array(size_pred)

    # 计算相关性与回归
    r, pvalue = pearsonr(size_gt, size_pred)
    slope, intercept, r_value, p_value, std_err = linregress(size_gt, size_pred)

    x = np.linspace(size_gt.min(), size_gt.max(), 200)
    y = slope * x + intercept
    ...
    plt.scatter(size_gt, size_pred, facecolors='none', edgecolors='blue', s=40)
    plt.plot(x, y, color='red', lw=2)
    plt.fill_between(x, y - y_err, y + y_err, color='red', alpha=0.15)

    plt.text(
        0.05, 0.95,
        f"$R$ = {r:.2f},  $p$ < {pvalue:.1e}",
    ...
    if fixed_range is not None:
        ...
        min_val = min(size_gt.min(), size_pred.min())
        max_val = max(size_gt.max(), size_pred.max())
    ...
    # ---- 保持固定布局 ----
    plt.axis('square')        # 保持坐标正方形比例
    plt.grid(False)
    plt.subplots_adjust(0, 0, 1, 1)  # 去除额外边距
    plt.savefig(save_dir, dpi=dpi, bbox_inches="tight", pad_inches=0.05)
    plt.close()
