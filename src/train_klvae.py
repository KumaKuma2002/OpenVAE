...
from utils_discriminator import MultiScaleStyleGANDiscriminator, discriminator_hinge_loss_multiscale, generator_adv_loss_multiscale
...
def parse_args():
    ...
    parser.add_argument("--discriminator_type", type=str, default="PatchGAN", help="Type of discriminator to use")
...
def main():
    ...
    """
    if args.discriminator_type == "StyleGAN":
        discriminator = MultiScaleStyleGANDiscriminator(in_channels=3, base_ch=64).to(accelerator.device)
        ...
        adv_loss_fn = generator_adv_loss_multiscale
        dis_hinge_loss_fn = discriminator_hinge_loss_multiscale
    else:
        """
        the default patch GAN
        """
        ...
        adv_loss_fn = generator_adv_loss
        dis_hinge_loss_fn = discriminator_hinge_loss
    ...
    if resume_from_checkpoint:
        if resume_from_checkpoint != "latest":
            ...
            # Get the most recent checkpoint under output_dir
            if os.path.isdir(args.output_dir):
                dirs = [d for d in os.listdir(args.output_dir) if d.startswith("checkpoint")]
                dirs = sorted(dirs, key=lambda x: int(x.split("-")[1]))
                last = dirs[-1] if dirs else None
                resume_path = os.path.join(args.output_dir, last) if last else None
            else:
                resume_path = None
        ...
        if not resume_path or not os.path.isdir(resume_path):
            ...
            try:
                disc_path = os.path.join(resume_path, "discriminator.pth")
                disc_ckpt = torch.load(disc_path, map_location="cpu")
                accelerator.unwrap_model(discriminator).load_state_dict(disc_ckpt)
                logger.info(f"\nSuccessfully loaded discriminator from {disc_path}\n")
            except Exception as e:
                logger.warning(f"\nFailed to load discriminator: {e}. Starting from scratch.\n")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model): # for accumulation steps
                ...
                """
                adv_loss = adv_loss_fn(discriminator, reconstructed)
                ...
                # adapt_w = compute_adaptive_gan_weight(reconstruction_loss, adv_loss, last_layer)
                # loss = reconstruction_loss + args.kl_weight * kl_loss \
                #     + args.seg_loss_weight * segmentation_loss \
                #     + adapt_w * adv_loss   # replaces static gan_loss_weight with adaptive weight
